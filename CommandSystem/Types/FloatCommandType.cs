...
{
    public class FloatCommandType : CommandType
    {
        public override CommandItemConvertResult Convert(CommandItem item, CommandExecuteData data)
        {
            ...
            {
                float intResult = item.InputValue.ToSingle();
                var attributes = item.CommandHeader.Attributes as FloatTypeAttribute;
                if (attributes.MinUsed && attributes.MaxUsed && (intResult < attributes.Min || intResult > attributes.Max))
                {
                    result.ErrorMessage = item.Container.GetText("ERR_VALUE_BETWEEN", attributes.Min, attributes.Max);
                    return result;
                }
